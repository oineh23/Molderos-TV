...
let currentPage = 1, tvPage = 1, animePage = 1, pinoyPage = 1, koreanPage = 1;
...
function loadMoreItems(type, containerId, page) {
  ...
  fetch(url)
    ...
    .catch(err => console.error(`Failed to load more from ${containerId}:`, err));
}

document.addEventListener("DOMContentLoaded", () => {
  const loadMoreConfig = [
    { id: "loadMoreMoviesBtn", type: "movie", container: "movies-list", page: () => ++currentPage },
    { id: "loadMoreTVBtn", type: "tv", container: "tvshows-list", page: () => ++tvPage },
    { id: "loadMoreAnimeBtn", type: "movie", container: "anime-list", page: () => ++animePage },
    { id: "loadMorePinoyBtn", type: "movie", container: "pinoy-movie-list", page: () => ++pinoyPage },
    { id: "loadMoreKoreanBtn", type: "movie", container: "korean-movie-list", page: () => ++koreanPage },
  ];

  loadMoreConfig.forEach(({ id, type, container, page }) => {
    const btn = document.getElementById(id);
    if (btn) {
      btn.addEventListener("click", () => {
        loadMoreItems(type, container, page());
      });
    }
  });
});
