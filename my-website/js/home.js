...
// == Trending TV Shows with Genre Filtering ==

const tvShowsList = document.getElementById("tvshows-list");
const tvGenreFilter = document.getElementById("tv-genre-filter");
const TMDB_API_KEY = 'b8c2d0fa80cd79b5d28d9fe2853806bb'; // 🔁 Replace this with your actual TMDB API key

// Function to fetch and display trending or filtered TV shows
async function fetchTrendingTVShows(genreId = "") {
  tvShowsList.innerHTML = ""; // Clear existing
  ...
  try {
    ...
    if (!data.results || data.results.length === 0) {
      tvShowsList.innerHTML = "<p>No TV shows found for this genre.</p>";
      return;
    }
    ...
    data.results.forEach(show => {
      ...
      tvShowsList.appendChild(card);
    });
  } catch (err) {
    console.error("Failed to fetch TV shows:", err);
    tvShowsList.innerHTML = "<p>Error loading TV shows. Please try again later.</p>";
  }
}

// Event listener for genre filter
if (tvGenreFilter) {
  tvGenreFilter.addEventListener("change", () => {
    const selectedGenre = tvGenreFilter.value;
    fetchTrendingTVShows(selectedGenre);
  });
}
