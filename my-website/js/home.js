...
const sectionElements = document.querySelectorAll('.scrollable-row');
const loadingState = {};

async function fetchMoreContent(section) {
  const type = section.dataset.type;
  const category = section.dataset.category;
  let page = parseInt(section.dataset.page || '1');

  if (loadingState[section.id]) return;
  ...
  if (category === 'trending') {
    ...
    url = `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_keywords=210024|287501&page=${page}`;
  } else if (category === 'vivamax') {
    url = `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_original_language=tl&page=${page}`;
  } else if (category === 'korean') {
    url = `https://api.themoviedb.org/3/discover/movie?api_key=${apiKey}&with_original_language=ko&page=${page}`;
  }
  ...
  try {
    ...
    section.dataset.page = page + 1;
  } catch (err) {
    console.error(`Error loading section ${section.id}:`, err);
  } finally {
    loadingState[section.id] = false;
  }
}

function isInViewport(element) {
  const rect = element.getBoundingClientRect();
  return rect.bottom <= (window.innerHeight + 200); // Load before bottom
}

function handleScrollLoad() {
  sectionElements.forEach(section => {
    if (isInViewport(section)) {
      fetchMoreContent(section);
    }
  });
}

document.addEventListener('DOMContentLoaded', () => {
  sectionElements.forEach(section => fetchMoreContent(section));
